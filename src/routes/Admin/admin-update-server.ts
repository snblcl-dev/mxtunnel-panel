...
const schema = z.object({
  ...
  slowdns_nameserver: z.string().optional(),
  slowdns_public_key: z.string().optional(),
  slowdns_dns: z.string().optional(),
...
export default {
  ...
  handler: async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const { action } = body;
    const server = await prisma.server.findFirst({ where: { id: Number(sid), user_id: id } });
    ...
    if (action === 'toggle') {
      ...
      await prisma.server.update({
        ...
        data: {
          ...
          slowdns_nameserver: orEmpty(d.slowdns_nameserver), slowdns_public_key: orEmpty(d.slowdns_public_key),
          slowdns_dns: orEmpty(d.slowdns_dns),
    ...
    return ajaxOrRedirect(req, reply, `/admin/users/${id}/config`, 'Servidor actualizado');
  },
} as RouteOptions;
